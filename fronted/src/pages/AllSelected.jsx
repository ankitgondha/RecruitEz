...
export function AllSelected() {
    ...
    useEffect(() => {
        const fetchInterviewCandidates = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/jobs/selected/${recruiterId}`);
    ...
    async function fetchCandidateDetails(candidateId) {
        try {
            const response = await fetch(`http://localhost:8080/jobs/candidate/${candidateId}`);
            if (!response.ok) {
    ...
    async function enhanceCandidates() {
        for (let i = 0; i < candidates.length; i++) {
            const candidateDetails = await fetchCandidateDetails(candidates[i].candidateId);
            if (candidateDetails) {
                ...
                <header className="flex h-14 items-center gap-4 border-b bg-muted/40 px-4 lg:h-[60px] lg:px-6">
                    ...
                    <div className="w-full flex-1">
                        <form>
                            <div className="relative">
                                <div className="flex items-center gap-2 font-semibold">
                                    <span className="">Selected Candidates</span>
                                </div>
                            ...
                            <TabsContent value="week">
                                <Card>
                                    <CardHeader className="px-7">
                                        <CardTitle>Candidates Selected</CardTitle>
                                        <CardDescription>
                                            ...
                                            <TableBody>
                                                ...
                                                {candidates && candidates.length > 0 ? (
                                                    candidates.map((candidate, index) => (
                                                        <TableRow>
                                                            <TableCell>
                                                                <div className="font-medium">{candidate.candidateName ? candidate.candidateName : "Unknown"}</div>
